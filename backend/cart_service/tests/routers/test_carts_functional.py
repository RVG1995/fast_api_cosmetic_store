...
async def test_enrich_cart_with_product_data_empty():
    ...
    cart = None
    result = await carts.enrich_cart_with_product_data(cart)
    assert result.id == 0
    assert result.items == []
...
async def test_get_cart_with_items_user_none():
    ...
    session = AsyncMock()
    result = await carts.get_cart_with_items(session, None)
...
async def test_get_cart_with_items_user_with_id(monkeypatch):
    ...
    result = await carts.get_cart_with_items(session, user)
    assert result.id == 1