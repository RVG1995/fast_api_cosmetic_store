"""Модуль аутентификации и авторизации для сервиса уведомлений."""
...
async def get_current_user(
    ...
    """Получает текущего пользователя из куки или заголовка Authorization. Возвращает User или None."""
    ...
    logger.debug("Cookie token: %s", token[:10] + "..." if token else None)
    logger.debug("Authorization header: %s", authorization[:10] + "..." if authorization else None)
    ...
    try:
        ...
        logger.debug("Decoded JWT, user_id: %s", user_id)
    ...
    try:
        ...
        if resp.status_code != 200:
            logger.warning("Auth service returned %s: %s", resp.status_code, resp.text)
