...
Этот модуль предоставляет функции для аутентификации пользователей через JWT токены,
проверки прав доступа и управления сессиями.
"""
...
from fastapi import (
    Depends, HTTPException, status, Cookie, Request, Header
)
...
SECRET_KEY = os.getenv(
    "JWT_SECRET_KEY",
    "zAP5LmC8N7e3Yq9x2Rv4TsX1Wp7Bj5Ke"
...
class User:
    ...
    def __init__(
        ...
        Args:
            ...
            is_super_admin: Флаг прав супер-администратора
...
async def get_current_user(
    ...
    Returns:
        Объект пользователя или None, если пользователь не аутентифицирован
    """
    logger.info(
        "Запрос авторизации: %s %s",
        request.method,
        request.url.path
    )
    ...
    if not access_token and authorization:
        if authorization.startswith("Bearer "):
            ...
            logger.info(
                "Токен получен из заголовка Authorization: %s...",
                access_token[:20]
            )
        else:
    ...
    try:
        ...
        logger.info(
            "Токен декодирован успешно: user_id=%s, is_admin=%s, is_super_admin=%s",
            user_id,
            is_admin,
            is_super_admin
        )
