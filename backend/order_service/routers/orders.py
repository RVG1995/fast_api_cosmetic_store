"""
API эндпоинты для управления заказами, включая создание, получение и обновление статуса.
Обрабатывает как обычные операции пользователей, так и административные
операции для сервиса заказов.
"""
...
from schemas import (
    OrderCreate, OrderResponse, OrderDetailResponse, PaginatedResponse,
    OrderStatistics, PromoCodeResponse, OrderResponseWithPromo, 
    OrderDetailResponseWithPromo
)
from services import (
    create_order, get_order_by_id, get_orders, cancel_order, 
    ...
    try:
        ...
        if user_id:
            try:
                ...
                logger.warning("Не удалось очистить корзину пользователя: %s", str(e))
        ...
        if loaded_order.promo_code_id:
            ...
            if promo_code:
                ...
                logger.info(
                    "Для заказа %s загружен промокод %s", 
                    loaded_order.id,
                    promo_code.code
                )
        ...
        logger.info(
        "Кэш заказа %s и связанных списков инвалидирован перед возвратом ответа",
        order.id)
...
async def get_order(
    ...
    try:
        ...
        if current_user.get("is_service"):
            logger.info(
                "Внутренний запрос от сервиса %s для заказа %s", 
                current_user.get('service_name'),
                order_id
            )
            # None означает получение заказа без проверки принадлежности конкретному пользователю
            user_id = None
        else:
        ...
        order = await get_order_by_id(
            session=session,
            order_id=order_id,
            # Если user_id=None и запрос от сервиса
            # то заказ будет получен без проверки пользователя
            user_id=user_id
        )
...
async def reorder_endpoint(
    order_id: int = Path(..., ge=1),
    personal_data_agreement: bool = Body(
        ...,
        embed=True,
    ...
    try:
        ...
        await get_products_info(product_ids, token)
