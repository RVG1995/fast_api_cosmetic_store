"""
Сервисные функции для работы с заказами, включая создание, обновление, отмену и получение статистики.
"""
import logging
...
async def create_order(
    ...
    for item in order_data.items:
        ...
        if not is_available:
            if not product_info:
                ...
                logger.error("Недостаточное количество товара %s: запрошено %s, доступно %s", 
                            product_id, quantity, product_info.stock)
    ...
    for item in order_data.items:
        ...
        if not success:
            logger.warning("Не удалось обновить количество товара %s", product_id)
...
async def get_order_by_id(session: AsyncSession, order_id: int, user_id: Optional[int] = None) -> Optional[OrderModel]:
    ...
    """
    # Логирование запроса
    logger.info("Запрос получения заказа: order_id=%s, user_id=%s", 
                order_id, user_id if user_id is not None else 'None (admin)')
    ...
    try:
        ...
        # Логируем результат
        if order:
            logger.info("Заказ найден: order_id=%s, user_id=%s, status=%s", 
                        order_id, order.user_id, order.status_id if hasattr(order, 'status_id') else 'N/A')
            ...
            logger.warning("Заказ с ID %s не найден. Фильтр по пользователю: %s", 
                          order_id, user_id is not None)
            ...
            if check_order:
                logger.warning("Заказ с ID %s существует, но не доступен для пользователя %s. Владелец заказа: %s", 
                              order_id, user_id, check_order.user_id)
...
async def change_order_status(
    ...
    if order.status_id == new_status.id:
        logger.warning("Попытка установить тот же статус (%s) для заказа %s. Операция отменена.", 
                      new_status.id, order_id)
...
async def update_order_items(
    ...
    """
    logger.info("Запрос получения заказа: order_id=%s, user_id=%s (admin)", order_id, user_id)
    ...
    logger.info("Заказ найден: order_id=%s, user_id=%s, status=%s", 
                order.id, order.user_id, order.status_id)
    logger.info("Количество товаров в заказе: %s", len(order.items))
...
async def check_promo_code(
    ...
    # Проверяем, использовал ли пользователь промокод по user_id
    if user_id is not None:
        already_used_by_user = await PromoCodeUsageModel.check_usage_by_user(session, promo_code.id, user_id)
        if already_used_by_user:
            logger.warning("Промокод %s уже использован пользователем с ID %s", code, user_id)
            return False, "Вы уже использовали этот промокод", None
