...
const AdminPaymentStatuses = () => {
  ...
  // Функция для загрузки статусов оплаты
  const loadStatuses = async () => {
    ...
    try {
      ...
      const response = await axios.get(`${ORDER_SERVICE_URL}/payment-statuses`, config);
      
      // Сортируем статусы по порядку сортировки
      const sortedStatuses = response.data.sort((a, b) => a.sort_order - b.sort_order);
      setStatuses(sortedStatuses);
    } catch (err) {
      console.error('Ошибка при загрузке статусов оплаты:', err);
      setError('Не удалось загрузить статусы оплаты. Пожалуйста, попробуйте позже.');
    } finally {
      setLoading(false);
    }
  };
    ...
    try {
      ...
      if (modalMode === 'create') {
        ...
        response = await axios.post(
          `${ORDER_SERVICE_URL}/payment-statuses`, 
          formData, 
          config
        );
        setSuccess('Статус оплаты успешно создан');
      } else {
        // Редактирование существующего статуса
        response = await axios.put(
          `${ORDER_SERVICE_URL}/payment-statuses/${selectedStatus.id}`, 
          formData, 
          config
        );
        setSuccess('Статус оплаты успешно обновлен');
      }
      ...
      console.error('Ошибка при сохранении статуса оплаты:', err);
      
      if (err.response?.data?.detail) {
        setError(err.response.data.detail);
      } else {
        setError('Не удалось сохранить статус оплаты. Пожалуйста, попробуйте позже.');
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      await axios.delete(
        `${ORDER_SERVICE_URL}/payment-statuses/${statusToDelete.id}`, 
        config
      );
      ...
      setSuccess('Статус оплаты успешно удален');
      
      // Автоматически скрываем сообщение об успехе через 3 секунды
      setTimeout(() => setSuccess(null), 3000);
    } catch (err) {
      console.error('Ошибка при удалении статуса оплаты:', err);
      
      if (err.response?.status === 400 && err.response?.data?.detail) {
        // Скорее всего, статус используется в заказах
        setError(err.response.data.detail);
      } else {
        setError('Не удалось удалить статус оплаты. Пожалуйста, попробуйте позже.');
      }
  ...
  return (
    <div className="container py-4">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h2>Управление статусами оплаты</h2>
        <Button 
          variant="primary" 
              ...
              <tbody>
                {statuses.map(status => (
                  <tr key={status.id}>
                    ...
                    <td>{status.is_paid ? 'Оплачено' : 'Не оплачено'}</td>
                    <td>
                      <Button 
      ...
      <Modal show={showModal} onHide={handleCloseModal} backdrop="static">
        <Modal.Header closeButton>
          <Modal.Title>
            {modalMode === 'create' ? 'Создание статуса оплаты' : 'Редактирование статуса оплаты'}
          </Modal.Title>
        </Modal.Header>
          ...
          <Modal.Body>
            ...
            <Form.Group className="mb-3">
              <Form.Check 
                ...
                id="is-paid"
                name="is_paid"
                label="Считается оплаченным"
                checked={formData.is_paid}
                onChange={handleInputChange}
              />
              <Form.Text className="text-muted">
                Если отмечено, то заказы с этим статусом оплаты считаются оплаченными
              </Form.Text>
            </Form.Group>
